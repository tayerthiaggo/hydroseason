...
def test_cli_fetch_era5_routes_to_aoi_wrapper_with_metadata(tmp_path: Path, monkeypatch):
    ...
    assert calls["aoi"]["large_era5_fallback"] == "ask"
...
def test_cli_fetch_auto_routes_to_aoi_fetch(tmp_path: Path, monkeypatch):
    ...
    assert calls["auto"]["large_era5_fallback"] == "ask"


def test_cli_fetch_era5_uses_default_path(tmp_path: Path, monkeypatch):
    from hydroseason import cli as cli_mod

    out_csv = tmp_path / "era5_default_fetch.csv"
    calls: dict[str, object] = {}

    monkeypatch.setattr("hydroseason.fetch.load_vector", lambda path: path)
    ...
    assert out_csv.exists()
    assert calls["aoi"]["source"] == "era5"
    assert calls["aoi"]["era5_zarr_path"] is None


def test_cli_fetch_large_era5_fallback_override(tmp_path: Path, monkeypatch):
    from hydroseason import cli as cli_mod
    ...
    assert rc == 0
    assert calls["aoi"]["large_era5_fallback"] == "allow"
