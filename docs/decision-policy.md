...
The v0.2.0 policy design is frozen in [`decision-policy-0.2.0.md`](decision-policy-0.2.0.md). Timing identifiability completed its synthetic and cohort gates as documented in that design and in the checked-in timing cohort comparison report (`case_studies/results/timing-identifiability/comparison-report.json`, false precise-boundary Wilson upper bound `1.2e-4 <= 0.05`), but recurrence identifiability remains shipped under its reviewed candidate scope recorded in `docs/calibration/2026-09-03-recurrence-identifiability-promotion.json`: `annual_shape_match` is the runtime narrowing rule, and promotion to `established_0_2_0` remains withheld until the blinded recurrence cohort gate is satisfied.
In the frozen release, HydroSeason still ships a single runtime method `hydroseason-v0.2.0`, and reports/manifests emit the runtime `decision_policy` value `hydroseason_0_2_0`.
`established_0_1_1` remains the historical baseline these gates were measured against.
