...
### Masking one untrusted month is not a sensitivity test

`direct_profile_combined` gives `quality_state == "low"` months zero weight,
so masking one cannot change the fit it produces. The only thing masking
changes is the span's contiguity, which re-routes it through the separate
gap-handling path — and the differing answer that came back was counted as
instability and vetoed the refinement. Daly River HY2005 is the case: eight
scenarios agreed on November and the cycle abstained regardless.

Those per-month masking scenarios are now skipped for candidates that
already exclude untrusted months from their own fit. Two things are
deliberately **kept**: the scenario that masks every untrusted month *at
once*, and the bound-substitution scenarios. The first is the only check on
a cycle whose entire trough is cloud-flagged — the fit then rests on the
trusted shoulders and finds a shallower low state that looks perfectly well
supported, which nothing else would catch (dropping it turned a test
fixture from `unresolved` straight to `confirmed`). The second still probes
what the cloud could be hiding, through the upstream scale estimate.
...
Measured across all seven catchments (84 cycles): refinement applied rises
from 66 to 77, with eleven cycles changing and only four moving a date —
Daly HY2005 (Sep→Nov), Fitzroy HY2010 (Aug→Dec), and Kakadu HY2007 and
HY2012 (both Oct→Nov). Each is the last month of a genuine tie. The other
seven changes are status-only: the same date, with the refinement now
confirming pass 1 rather than abstaining. `shape_fit` is unchanged on all
84 cycles.
...
60 cycles compared, 33 identical between candidates, 27 differ. This count
moved across the fixes above: 45/15 originally, 41/19 after the
quality-reliability fallback and gap-path guard, back to 45/15 under the
proportional equivalence margin (not a return to the old behaviour — both
candidates simply landed on the cycle's own trough more often), and finally
33/27 once the masking change let `direct_profile_combined` resolve eleven
cycles it had previously abstained on. Divergence from `shape_fit` rising
is expected here: the two candidates now answer different numbers of
cycles, so the comparison counts more genuine disagreements rather than
matching abstentions. Of the nine Daly examples discussed during review
(`daly_nine_example_disposition.csv`), only HY2011 now agrees with
`shape_fit` — both abstain on it, for the reason recorded above. The
candidate resolves HY2005 (`2005-11`), HY2018 (`2018-11`) and HY2020
(`2020-11`) where `shape_fit` abstains, and abstains on HY2012 where
`shape_fit` publishes a `2013-02` the reviewer rejected. HY2016 differs at
`2016-11` against `shape_fit`'s frozen `2016-12` (see the gap-path fix
above; a newly-correct disagreement, not a regression). The rest were part
of the blinded review above — 2008
...
now publishes `2014-11` — the month the reviewer preferred — over
`shape_fit`'s `2014-12`), and 2019
