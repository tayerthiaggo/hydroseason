"""Direct-profile low-state-departure research candidate. Not production code.

Implements the Step 3 numerical specification
(``docs/superpowers/specs/2026-09-09-low-state-direct-profile-numerics.md``):
profile the equivalence-state departure ``T(f, L, delta)`` directly over a
bounded grid of the low-state reference level ``L``, rather than mapping a
finite set of best trough fits through one fixed reference level.

``refine_trough_span_direct_profile`` is read-only against
``hydroseason._trough_refinement``: no module attribute is reassigned
(dependency injection only -- see numerics spec, section 8).
``refine_trough_span_direct_profile_with_sensitivity`` is the one exception:
it temporarily reassigns ``_refine_selected_span`` (restored in ``finally``)
so this candidate can reuse production's real peak/quality sensitivity
ensemble (``refine_trough_span``'s outer loop), exactly as the first
prototype (``endpoint_experiment.py``) already does. Single-threaded only;
never imported by production HydroSeason code.
"""
...
import hydroseason._trough_refinement as _core  # noqa: E402
from hydroseason._trough_refinement import (  # noqa: E402
    PeakBoundary,
...
_ORIGINAL_REFINE_SELECTED_SPAN = _core._refine_selected_span
...
def final_departure_index(
    ...
    *,
    ...
    # The contiguous low run touching `candidate` must not reach either edge.
    run_start = candidate
...
class DirectProfileSolution:
    ...
    fit_at_best: DirectProfileFit | None = None
...
def _natural_reference_level(
    ...
    contract's common-reference rule (Step 1 section 8): the reference must
    not be re-derived per candidate block, or a sequence of small increases
    could chain into a drifting low state.
    """
    n = values.size
    best = None
...
def solve_direct_profile(
    ...
    *,
    ...
    best_by_departure: dict[int, DirectProfileFit] = {}
    ...
    # optimum, not merely with the best among already-departure-labelled fits
    # (numerics spec section 2.4; see also the flat-series negative control).
...
def _refine_gap_direct_profile(
    ...
    *,
    ...
    checks) -- see numerics spec section 4's compatibility argument. The one
    change is the post-gap comparisons: ``value <= L + delta_pp`` (this
    candidate's target) instead of the raw Huber-loss profile cutoff (the
    old target's unit).
    """
    ...
    if bool(at_low_level[0]):
        return TroughRefinementResult(
            ...
            policy_version=VERSION, loss_basis=loss_basis,
        )
    ...
    return TroughRefinementResult(
        ...
        effective_support=float(np.sum(before_weights)), policy_version=VERSION, loss_basis=loss_basis,
    )
...
def refine_trough_span_direct_profile(
    ...
    *,
    ...
    Returns the existing ``TroughRefinementResult`` shape unchanged. Never
    called from production ``analyze_hydrological_state``/
    ``detect_dynamic_hydrological_years`` paths.
    """
    if right_peak is None:
        return _empty(policy, "awaiting_next_peak", "open_span")
    if (
        ...
        return _empty(policy, "unavailable", "missing_or_unresolved_peak")
    ...
    if len(span) < 3 or not bool(observed[0]) or not bool(observed[-1]):
        return _empty(policy, "unresolved", "no_defensible_low_state")
    if not bool(observed.all()):
        return _refine_gap_direct_profile(
            span, values_series, weights, observed,
            policy=policy, delta_pp=delta_pp, measurement_tolerance_pp=measurement_tolerance_pp,
        )
    ...
    preliminary = [candidate for candidate in preliminary if candidate.converged]
    if not preliminary:
        return _empty(policy, "unresolved", "no_converged_candidate")
    preliminary_best = min(
    ...
    if scale_mode == "combined":
        scale = _combined_scale(
            values, residual_scale, span, measurement_tolerance_pp=measurement_tolerance_pp,
        )
    ...
    solution = solve_direct_profile(
        values, weights, delta_pp=delta_pp, scale=scale, huber_k=policy.huber_k,
        profile_loss_cutoff=policy.profile_loss_cutoff, l_uncertainty_k=l_uncertainty_k,
    )
    if solution.departure_position is None:
        return TroughRefinementResult(
            ...
            policy_version=VERSION, loss_basis=solution.loss_basis,
        )
    ...
    # lets the solver's fit absorb a pulse via pooling rather than forcing it
    # into a flat block (see the pulse-return integration test).
    best_fit = solution.fit_at_best
    assert best_fit is not None
    best_start = best_fit.start_position
    # `_pulse_dates` (production, reused unmodified) reads `fit.end_position`
    # as the boundary index. In this candidate the departure position can sit
    # past the winning block's own `end_position` (the low run extends into
    # an isotonic branch that absorbed a pulse via pooling -- see the
    # pulse-return integration test), so the fit handed to `_pulse_dates`
    # must carry the resolved departure as its end position, not the block's.
    ...
    return TroughRefinementResult(
        ...
        effective_support=float(np.sum(weights)), policy_version=VERSION,
        loss_basis=solution.loss_basis,
    )


def _empty(policy: TroughRefinementPolicy, status: str, reason: str) -> TroughRefinementResult:
    return TroughRefinementResult(
        status=status, reason=reason, boundary=None, boundary_candidates=(),
        low_state_start=None, low_state_end=None, recovery_start=None,
        pulse_months=(), local_scale_pp=0.0, best_loss=float("nan"),
        effective_support=0.0, policy_version=VERSION,
    )


def refine_trough_span_direct_profile_with_sensitivity(
    frame: pd.DataFrame,
    *,
    left_peak: PeakBoundary,
    right_peak: PeakBoundary | None,
    ...
    """Direct-profile challenger under production's full sensitivity ensemble.

    ``refine_trough_span_direct_profile`` alone only answers "what is the
    direct-profile departure for these exact peaks and this exact quality
    record" -- it never re-runs the peak-date and quality perturbation
    scenarios ``refine_trough_span`` (the real production entry point) uses
    to decide `provisional`/`unresolved`/`unstable_*_sensitivity`. Comparing
    it directly against ``refine_trough_span`` on real data is therefore not
    apples-to-apples: production's answer already reflects that robustness
    check, this candidate's bare answer does not.

    This function closes that gap the same way the first prototype
    (``endpoint_experiment.py::refine_experimental``) did: temporarily
    substitute ``_refine_selected_span`` and let production's own
    ``refine_trough_span`` drive the full ensemble on top of it. Single-
    threaded; raises if called re-entrantly in the same interpreter.
    """
    if _core._refine_selected_span is not _ORIGINAL_REFINE_SELECTED_SPAN:
        raise RuntimeError("requires a single-thread, unnested call")

    def selected(span_frame, **kwargs):
        return refine_trough_span_direct_profile(
            span_frame, **kwargs, delta_pp=delta_pp,
            l_uncertainty_k=l_uncertainty_k, scale_mode=scale_mode,
        )

    _core._refine_selected_span = selected
    try:
        return _core.refine_trough_span(
        ...
        _core._refine_selected_span = _ORIGINAL_REFINE_SELECTED_SPAN
